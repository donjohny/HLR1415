...
calculate (struct calculation_arguments* arguments, struct calculation_results *results, struct options* options)
{
	...
	while (options->term_iteration > 0)
	{
		...
		/* over all columns (y) instead of rows */
		for (i = 1; i < N; i++)
		{
			/* over all rows (x) instead of columns */
			for (j = 1; j < N; j++)
			{
				...
				{
					residuum = ((TWO_PI_SQUARE * sin((double)(j) * PI * arguments->h) * sin((double)(i) * PI * arguments->h) * arguments->h * arguments->h - star) / 4.0);
				}
				
				korrektur = residuum;
				residuum = (residuum < 0) ? -residuum : residuum; //Betrag von Residuum
